            ...
            <div class="col-md-9">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title">Grupo: XPTO</h3>
                    </div>
                    <div class="panel-body">
                        ...
                        <table class="table table-striped table-hover ">
                          ...
                          <?php
                            ...
                            $sql = " SELECT COUNT(*) AS total FROM (PRODUTO ".
                                   " INNER JOIN PRODUTOGRUPO AS PG ON (PG.SUBGRUPO_CONTROLE = {$_subgrupo} AND PG.PRODUTO_CONTROLE = PRODUTO.PRODUTO_CONTROLE AND PG.STATUS = 1)) ";
                            ...
                            WHERE STATUS = 1 AND PRODUTO.PRODUTO_CONTROLE In 
                                  (
                                    ...
                                    FROM [
                                           ...
                                           FROM PRODUTO
                                           INNER JOIN PRODUTOGRUPO AS PG ON (PG.SUBGRUPO_CONTROLE = {$_subgrupo} AND PG.PRODUTO_CONTROLE = PRODUTO.PRODUTO_CONTROLE AND PG.STATUS = 1)
                                           WHERE PRODUTO.STATUS = 1 ORDER BY PRODUTO.NOME, PRODUTO.PRODUTO_CONTROLE
                                         ]. AS A
                                    WHERE STATUS = 1 ORDER BY A.NOME DESC, A.PRODUTO_CONTROLE
                                  )
                            ORDER BY PRODUTO.NOME, PRODUTO.PRODUTO_CONTROLE";
                            $result = $pdo->query($sql);
                            if($result){
                            ...
                            while ($row = $result->fetch()) {
                                ...
                                $sql_grp = "SELECT s.NOME, g.NOME AS GRUPO_NOME FROM (SUBGRUPO AS s".
                                           " INNER JOIN PRODUTOGRUPO AS p ON (p.SUBSUBGRUPO_CONTROLE = s.SUBSUBGRUPO_CONTROLE AND s.STATUS = 1))".
                                           " INNER JOIN GRUPO        AS g ON (s.SUBGRUPO_CONTROLE    = g.SUBGRUPO_CONTROLE AND g.STATUS = 1)".
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="4">
                                    <span class="pull-right"><?php paginacao(3, $p,$pags, $pesquisa, $_subgrupo); ?></span>
                                </td>
                            </tr>
                        </tfoot>
                        <?php
